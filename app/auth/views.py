...
from . import auth # Reverted from bp
from .. import db
from ..models import User
from .forms import LoginForm, RegistrationForm, RegistrationPasswordForm


@auth.route('/login', methods=['GET', 'POST']) # Reverted
def login():
...
@auth.route('/register', methods=['GET', 'POST']) # Reverted
def register():
    if current_user.is_authenticated:
        return redirect(url_for('main.index'))
    ...
    if not session.get('registration_verified'):
        ...
        if form.validate_on_submit():
            if form.registration_password.data == "Forefrontcsez@999": # Reverted
